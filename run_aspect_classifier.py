...
class DataProcessor(object):
  ...
  def _read_data(cls, input_file, quotechar=None):
    ...
    with open(input_file, "r", encoding="utf-8") as f:
      reader = csv.reader(f, delimiter="\t", quotechar=quotechar) #delimiter="\t" 加了分隔符会报错，所有数据成为了一列
      lines = []
      for line in reader:
        lines.append(line)
      return lines
...
class PerformanceProcessor(DataProcessor):
  """Processor for the MRPC data set (GLUE version)."""

  def get_train_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_data(os.path.join(data_dir, "Rel_data_train.txt")), "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_data(os.path.join(data_dir, "WOSingle_dev3.tsv")), "dev")

  def get_test_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_data(os.path.join(data_dir, "WOSingle_dev3.tsv")), "test")

  def get_labels(self):
    """See base class."""
    # “0”表示不属于此类别，“1”表示属于此类别
    return ["0", "1"]
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
        ...
        text_a = tokenization.convert_to_unicode(str(line[0]).strip('"'))
        text_b = tokenization.convert_to_unicode(line[1])
        # text_c = tokenization.convert_to_unicode(str(line[2]))
        if set_type == "test":
            label = "0"
        else:
        ...
        index += 1
    return examples
class UsabilityProcessor(DataProcessor):
  """Processor for the MRPC data set (GLUE version)."""

  def get_train_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_data(os.path.join(data_dir, "WOSingle_train.tsv")), "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_data(os.path.join(data_dir, "RelSingle_data_dev.tsv")), "dev")

  def get_test_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_data(os.path.join(data_dir, "RelSingle_data_dev.tsv")), "test")
  ...
  def _create_examples(self, lines, set_type):
    ...
    for (i, line) in enumerate(lines):
      ...
      text_a = tokenization.convert_to_unicode(str(line[0]).strip('"')+ str(line[1]))
      # text_a = tokenization.convert_to_unicode(str(line[0]).strip('"'))
...
class SecurityProcessor(DataProcessor):
  """Processor for the MRPC data set (GLUE version)."""

  def get_train_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_data(os.path.join(data_dir, "WOMul_train.tsv")), "train")

  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_data(os.path.join(data_dir, "WOSingle_dev_sec.tsv")), "dev")

  def get_test_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_data(os.path.join(data_dir, "WOSingle_dev_sec.tsv")), "test")
...
class OthersProcessor(DataProcessor):
  ...
  def get_dev_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_data(os.path.join(data_dir, "WOMul_dev.tsv")), "dev")

  def get_test_examples(self, data_dir):
    """See base class."""
    return self._create_examples(
        self._read_data(os.path.join(data_dir, "WOMul_test.tsv")), "test")
...
class SentimentProcessor(DataProcessor):
  ...
  def get_labels(self):
    """See base class."""
    # o"表示中立,“p”表示积极,""n"表示消极
    # 每一类代表的类别与此处定义的标签顺序相同
    return ["o", "p", "n"]
...
def main(_):
  ...
  processors = {
      ...
      "usability": UsabilityProcessor,
      "security": SecurityProcessor,
      "others": OthersProcessor,
