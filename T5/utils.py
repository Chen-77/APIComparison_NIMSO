...
def test(model: T5ForConditionalGeneration, device, data_loader, label_id_list: List):
    ...
    predicted_labels = torch.LongTensor([]).to(device)
    ground_truth_labels = torch.LongTensor([]).to(device)
    predicted_probs = torch.FloatTensor([]).to(device) #初始化
    data_loader = tqdm(data_loader, file=sys.stdout)
    for step, data in enumerate(data_loader):
        input_ids = data['input_ids'].to(device)
        attention_mask = data['attention_mask'].to(device)
        ...
        with torch.no_grad():
            ...
            # 一般来说，训练过的 T5 模型在做分类任务时，生成的标签不会出现标签外的单词，具体请看原论文。以下代码只是证明一下确实没有生成标签外的单词
            decoder_input_ids = model._shift_right(labels)  # T5模型要求将标签向右移一位作为decoder_input_ids
            outputs = model(input_ids=input_ids, attention_mask=attention_mask, decoder_input_ids=decoder_input_ids)
            logits = outputs.logits[:, 0, :]  # 获取logits
            pred_probs = torch.softmax(logits, dim=-1)[:, label_id_list[1]]  # 计算正类的概率
            pred_labels = logits.argmax(dim=-1)  # 获取预测标签
